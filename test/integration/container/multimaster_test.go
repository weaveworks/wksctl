...
func TestMultimasterSetup(t *testing.T) {
	...
	const capeiImage = "weaveworks/cluster-api-existinginfra-controller:v0.2.4-0.20210225200943-0badf2ca672c"
			...
			for i := 0; i < 4; i++ {
				for _, kubeletArg := range expectedKubeletArgs {
					...
					for _, c := range subCmds {
						cargs := "footloose -c " + filepath.Join(rootDir, "examples/footloose/"+node_os+node_version+"/docker/multimaster.yaml") + " ssh " + fmt.Sprintf("root@node%d", i) + " \"" + c + "\""
