...
func TestFullNameRegexp(t *testing.T) {
	if anchoredNameRegexp.NumSubexp() != 3 {
	...
	testcases := []regexpMatch{
		...
		},
		{
			...
			match: true,
			subs:  []string{"a", "a", strings.Repeat("a/", 126) + "a"},
		},
		{
			input: "a-/a/a/a",
			match: false,
...
func TestReferenceRegexp(t *testing.T) {
	fmt.Println(ReferenceRegexp)
	if ReferenceRegexp.NumSubexp() != 5 {
		t.Fatalf("anchored name regexp should have five submatches: %v, %v != 5",
